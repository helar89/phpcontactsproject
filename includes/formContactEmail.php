...
?>
	...
	<form method="POST" >
	<table>
	<tr><td><label class="contact-label" for="em_type">Email Type:</label></td>
		<td><select class="contact-input" id="em_type" name="em_type" size="1">
<?php if ($type == ""){ ?>
...
<?php } ?>
	...
	<tr><td><label class="contact-label" for="em_email">Email Address</label></td>
		<td><input class="contact-input" type="email" id="em_email" name="em_email" size="50" maxlength="50" value="<?php echo $email; ?>"></td>
	</tr>
	</table>
    <table>
    <tr>
...
function validateContactEmail(){
	...
	if(!isset($_POST['em_email'])){
        ...
        $emailPattern = "/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,3}$/"; // Use regex to pattern the email
        $result = preg_match($emailPattern, $email); // Use preg_match to check the email
		if (strlen($email) == 0){
			$err_msgs[] = "The email address field must not sbe empty";
		} else if (strlen($email) > 50){
			$err_msgs[] = "The email address is too long";
		} else if ($result == false) {
            $err_msgs[] = "The email address is incorrect";
        }
	}
	if (count($err_msgs) == 0){
		$_POST['em_type'] = $type;
